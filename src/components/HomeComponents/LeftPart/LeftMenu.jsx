  ...
  return (
    <>
      {SeparationSettings ? (
        ...
        <NavLink
          to={data.to}
          className={`flex items-center justify-center w-10 h-10 transition-all duration-200 ease-linear rounded-full cursor-pointer xl:justify-normal xl:flex xl:px-6 xl:py-3 lg:mb-5 xl:mb-7 hover:bg-black item-center xl:gap-x-5 group xl:w-auto xl:h-auto ${path === data.to ? 'bg-black':""}`}
        >
          
          <div className={`group-hover:text-white  ${path === data.to ? 'text-white':""}`}>
            <ItemIcon />
          </div>
          <div className={`hidden text-base xl:block font-gilroySemiBold group-hover:text-white  ${path === data.to ? 'text-white':""}`}>
            {data.title}
          </div>
        </NavLink>
      )}
